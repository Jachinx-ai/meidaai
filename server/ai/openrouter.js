...
function imagesMessage(text, imageDataUrls) {
  return [{
    role: "user",
    content: [
...
/* 图像生成（多张参考图 + 提示词 → 一张图的 dataURL） */
async function generateImageFromImages(model, prompt, imageDataUrls, { timeoutMs = 180000 } = {}) {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), timeoutMs);
  try {
    const resp = await fetch(API, {
      ...
      body: JSON.stringify({
        model,
        modalities: ["image", "text"],
        messages: imagesMessage(prompt, imageDataUrls),
      }),
      signal: ctrl.signal,
    });
    const data = await resp.json();
...
module.exports = { chat, generateImage, generateImageFromText, generateImageFromImages, imageMessage, imagesMessage, parseJson };
